...
export function AttackTimeline({ data }: AttackTimelineProps) {
    ...
    const CustomTooltip = ({ active, payload }: any) => {
        ...
        return (
            <div className="bg-sf-bg border border-sf-border p-3 shadow-2xl max-w-[300px]">
                <div className="font-mono text-[10px] text-sf-muted mb-1 flex justify-between gap-4">
                    <span>{new Date(dp.x).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
                    <span className={`font-bold ${dp.level === 'CRITICAL' ? 'text-sf-critical' : dp.level === 'WARN' ? 'text-[#ffee00]' : 'text-sf-text'}`}>
                        {dp.level}
                    </span>
                </div>
                {dp.tactic && (
                    <div className="font-mono text-[9px] text-sf-accent mb-1">⚡ {dp.tactic}</div>
                )}
                {dp.cep_sequence_id && (
                    <div className="font-mono text-[9px] text-[#f97316] mb-1">🔗 SEQ: {dp.cep_sequence_id.slice(0, 12)}…</div>
                )}
                <div className="font-mono text-[11px] text-sf-text mb-1">{dp.y}</div>
                <div className="font-mono text-[10px] text-sf-muted truncate">{dp.message}</div>
            </div>
        );
    };
    ...
    if (chartData.length === 0) {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center bg-sf-surface/20 border-t border-sf-border border-dashed p-8">
                <div className="w-1.5 h-1.5 bg-sf-muted/50 mb-3 rotate-45" />
                <span className="text-sf-muted tracking-widest text-[10px] font-mono uppercase">WAITING FOR TIMELINE DATA...</span>
            </div>
        );
    }

    return (
        <div className="w-full h-full bg-sf-bg overflow-hidden relative flex flex-col">
            {/* AI summary button */}
            <div className="absolute top-2 right-2 z-10 flex items-center gap-2">
                {aiSummary && (
                    <div className="bg-sf-surface/90 border border-sf-accent/30 px-3 py-2 text-sf-text text-[10px] font-mono max-w-[320px] shadow-lg">
                        {aiSummary}
                    </div>
                )}
                <button
                    ...
                    disabled={aiLoading}
                    className="flex items-center gap-1 px-2 py-1 bg-sf-accent/10 hover:bg-sf-accent/20 border border-sf-accent/30 text-sf-accent text-[9px] font-mono tracking-widest uppercase transition-colors disabled:opacity-50"
                >
            ...
            {tacticBoundaries.length > 0 && (
                <div className="shrink-0 flex gap-1 px-[108px] pb-1 overflow-x-hidden">
                    {tacticBoundaries.map(({ tactic }) => (
                        <span
                            key={tactic}
                            className="text-[8px] font-mono text-sf-accent/60 truncate bg-sf-accent/5 px-1 border-l border-sf-accent/20"
                        >
                            {tactic}
                        </span>
                    ))}
            ...
            {Object.keys(cepGroups).length > 0 && (
                <div className="shrink-0 flex gap-2 px-3 pb-1">
                    {Object.entries(cepGroups).slice(0, 5).map(([seqId, pts]) => (
                        ...
                        >
                            🔗 CEP {seqId.slice(0, 8)}… <span className="text-sf-muted">({pts.length} events)</span>
                        </span>
                    ))}
                </div>
            )}
