...
const SEV_BORDER: Record<string, string> = {
    critical: 'border-sf-critical/50',
    high:     'border-[#f97316]/50',
    medium:   'border-[#ffee00]/30',
    low:      'border-sf-border',
    info:     'border-sf-border',
};
...
export function AttackPatternCard({
    ...
    if (error) {
        return (
            <div className="border border-sf-critical/30 bg-sf-critical/5 p-4 text-sf-critical font-mono text-[10px]">
    ...
    if (groups.length === 0) {
        return (
            <div className="flex items-center justify-center h-24 text-sf-muted font-mono text-[10px] tracking-widest border border-dashed border-sf-border">
                ...
                {groups.map((group) => (
                    <motion.div
                        ...
                        className={`
                            ...
                            bg-sf-surface border ${SEV_BORDER[group.peak_severity] ?? 'border-sf-border'}
                            p-4 flex flex-col gap-3
                            hover:bg-sf-surface-raised transition-colors
                        `}
                    ...
                    >
                        ...
                        <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-1.5 min-w-0">
                                <ShieldAlert className="w-3.5 h-3.5 text-sf-accent shrink-0" />
                                <span className="font-mono text-[10px] text-sf-accent uppercase tracking-wider truncate">
                                    {group.tactic}
                                </span>
                            </div>
                            <Badge
                        ...
                        {/* Technique */}
                        {group.technique && (
                            <div className="font-mono text-[9px] text-sf-muted truncate flex items-center gap-1">
                        ...
                        {/* Bottom stats */}
                        <div className="flex items-center justify-between mt-auto pt-2 border-t border-sf-border/50">
                            <div className="flex items-center gap-1 text-sf-muted font-mono text-[9px]">
                        ...
                        <div className="absolute bottom-1 right-2 font-mono text-[7px] text-sf-muted/30 select-none">
