...
export function DataFreshness({ lastUpdated, refreshInterval = 10, showProgressBar = false }: DataFreshnessProps) {
 ...
 let stateColor = 'bg-ng-lime';
 if (lastUpdated) {
 if (secondsAgo > refreshInterval * 2) stateColor = 'bg-ng-error';
 else if (secondsAgo > refreshInterval * 0.8) stateColor = 'bg-ng-magenta';
 }

 return (
 <div className="flex flex-col gap-0.5 min-w-[70px]">
 <div className="flex items-center gap-1.5 justify-end">
 <span className="text-[9px] font-mono tracking-widest uppercase text-ng-muted">
 {displayTime}
 </span>
 <motion.span 
 ...
 {showProgressBar && (
 <div className="h-[1px] w-full bg-ng-mid/80">
 <motion.div 
