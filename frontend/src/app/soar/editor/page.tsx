...
const AVAILABLE_ACTIONS: { type: ActionType; icon: React.ReactNode; label: string; desc: string }[] = [
    { type: "isolate_host", icon: <ShieldAlert className="w-4 h-4 text-[var(--sf-critical)]" />, label: "Isolate Host", desc: "Block network access via EDR" },
    { type: "block_ip", icon: <ShieldAlert className="w-4 h-4 text-[var(--sf-warning)]" />, label: "Block IP", desc: "Add IP to firewall deny list" },
    { type: "wait_for_approval", icon: <UserCheck className="w-4 h-4 text-[var(--sf-accent)]" />, label: "Wait for Approval", desc: "Pause until analyst approves" },
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-8 relative">
                    <div className="max-w-3xl mx-auto">
                        <div className="mb-8">
                            <label className="text-xs font-bold text-sf-muted uppercase tracking-wider mb-2 block">Description</label>
                            <input 
                                type="text"
                                value={description}
                        ...
                        <div className="space-y-4 relative">
                            {/* Start Node */}
                            <div className="w-48 mx-auto bg-sf-surface border border-sf-border rounded-full py-2 px-4 flex items-center justify-center gap-2 shadow-lg z-10 relative">
                                <Play className="w-4 h-4 text-[var(--sf-safe)]" />
                ...
                <div className="w-80 bg-sf-bg border-l border-sf-border flex flex-col items-stretch overflow-y-auto">
                    <div className="p-4 border-b border-sf-border bg-sf-bg/80 sticky top-0 z-10 backdrop-blur-sm">
                        <h2 className="text-sm font-bold text-white uppercase tracking-wider">Available Actions</h2>
                        <p className="text-[10px] text-sf-muted mt-1 uppercase tracking-widest">Click to append to sequence</p>
                    </div>
                        ...
                        {AVAILABLE_ACTIONS.map(action => (
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-sf-muted leading-tight">{action.desc}</p>
                                </div>
                            </button>
                        ))}
                    </div>
