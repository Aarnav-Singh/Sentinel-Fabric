...
function TrendIcon({ trend }: { trend: 'up' | 'down' | 'stable' }) {
 if (trend === 'up') return <TrendingUp className="w-3 h-3 text-ng-lime" />;
 if (trend === 'down') return <TrendingDown className="w-3 h-3 text-ng-error" />;
 return <Minus className="w-3 h-3 text-ng-muted" />;
}

function CoverageDot({ coverage }: { coverage: MitreTechnique['coverage'] }) {
 const colors: Record<string, string> = {
 covered: 'bg-ng-lime',
 partial: 'bg-ng-magenta',
 blind: 'bg-ng-error',
 };
...
export default function PosturePage() {
 ...
 const scoreColor = compositeScore > 80 ? 'text-ng-lime' : compositeScore > 60 ? 'text-ng-magenta' : 'text-ng-error';
 ...
 <div>
 <h2 className="font-headline tracking-widest uppercase text-[10px] font-mono tracking-widest text-ng-muted uppercase">Global Posture Score</h2>
 ...
 <span className="text-[16px] text-ng-muted">/100</span>
 </div>
 </div>
 ...
 const ringColor = domainScore > 80 ? 'var(--ng-lime)' : domainScore > 60 ? 'var(--ng-magenta)' : 'var(--ng-error)';
 return (
 <PanelCard key={domain.id} className="p-3 flex flex-col justify-between relative">
 {domainsError && <span className="absolute top-2 left-2 text-[8px] bg-ng-magenta/20 border border-ng-magenta/40 text-ng-magenta px-1 font-mono">[DEMO]</span>}
 <div className="flex items-center justify-between">
 <span className={`text-[10px] font-mono font-bold uppercase text-ng-muted ${domainsError ? 'ml-10' : ''}`}>{domain.name}</span>
 <TrendIcon trend={domain.trend} />
 </div>
 <div className="text-3xl font-mono text-ng-on my-2" style={{ color: ringColor }}>
 {Math.round(domainScore)}
 </div>
 <div className="w-full h-1 bg-ng-mid border border-ng-outline-dim/40 relative">
 ...
 <div className="flex items-center justify-between mb-4 border-b border-ng-outline-dim/40 pb-2">
 <h3 className="text-[10px] font-mono uppercase tracking-widest text-ng-muted">MITRE ATT&amp;CK COVERAGE</h3>
 <div className="flex items-center gap-4 text-[9px] font-mono">
 <span className="flex items-center gap-1.5"><span className="w-2 h-2 bg-ng-lime inline-block border border-black/30" /> COVERED</span>
 <span className="flex items-center gap-1.5"><span className="w-2 h-2 bg-ng-magenta inline-block border border-black/30" /> PARTIAL</span>
 <span className="flex items-center gap-1.5"><span className="w-2 h-2 bg-ng-error inline-block border border-black/30" /> BLIND</span>
 </div>
 </div>
 <div className="overflow-x-auto">
 {tactics.length === 0 ? (
 <p className="text-ng-muted font-mono text-xs text-center py-4 uppercase tracking-widest">Connect data sources to populate matrix</p>
 ) : (
 <div className="space-y-4 min-w-[800px]">
 {tactics.map((tactic) => (
 <div key={tactic.tactic} className="flex flex-col">
 <p className="text-[9px] font-bold uppercase font-mono text-ng-muted mb-1.5">{tactic.tactic}</p>
 <div className="flex flex-wrap gap-1.5">
 {tactic.techniques.map((tech) => (
 <div
 key={tech.id}
 className="flex items-center gap-1.5 bg-ng-mid border border-ng-outline-dim/40 rounded-none px-1.5 py-0.5"
 title={tech.fix ?? tech.name}
 >
 <CoverageDot coverage={tech.coverage} />
 <span className="text-[9px] font-mono text-ng-on">{tech.id}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="px-4 py-3 border-b border-ng-outline-dim/40 flex justify-between items-center bg-ng-mid">
 <h3 className="text-[10px] font-mono uppercase tracking-widest text-ng-muted">REMEDIATION QUEUE</h3>
 </div>
 <div className="p-2">
 {findings.length === 0 ? (
 <p className="text-ng-muted font-mono text-[10px] p-4">NO REMEDIATION ITEMS</p>
 ) : (
 <DataGrid
 data={sortedFindings}
 rowKey="id"
 columns={[
 { header: "PRI", key: "priority", render: (val) => <span className="text-ng-muted font-mono">{val}</span> },
 { header: "FINDING", key: "title", render: (val, row) => (
 <div>
 <span className="font-medium text-ng-on">{val}</span>
 <span className="block text-[9px] text-ng-muted mt-0.5">{row.description}</span>
 </div>
 )},
 { header: "SEV", key: "severity", render: (val) => (
 <span className={`px-1.5 py-0.5 text-[9px] uppercase border ${val === 'critical' ? 'border-ng-error text-ng-error' : val === 'high' ? 'border-ng-magenta text-ng-magenta' : 'border-ng-outline-dim/40 text-ng-muted'}`}>{val}</span>
 )},
 { header: "EFFORT", key: "effort", render: (val) => <span className="text-ng-muted uppercase text-[9px]">{val}</span> },
 { header: "STATUS", key: "status", render: (val) => (
 <span className={`text-[9px] font-bold uppercase ${val === 'open' ? 'text-ng-error' : val === 'in_progress' ? 'text-ng-magenta' : 'text-ng-lime'}`}>
 {val.replace('_', ' ')}
 </span>
 )}
 ]}
