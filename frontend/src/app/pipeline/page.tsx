...
export default function MLPipelinePage() {
 ...
 <span className="text-[10px] text-ng-muted font-mono tracking-widest">AVG LATENCY</span>
 <div className="text-2xl font-mono text-ng-on">
 {pipelineStatus.avg_duration_ms != null && !isNaN(Number(pipelineStatus.avg_duration_ms)) ? (
 <>{pipelineStatus.avg_duration_ms}<span className="text-sm text-ng-muted ml-0.5">MS</span></>
 ) : (
 "—"
 )}
 </div>
 ...
 <PanelCard className="flex-1 p-3 flex flex-col gap-1">
 <span className="text-[10px] text-ng-muted font-mono tracking-widest">AVG ACCURACY</span>
 <div className="text-2xl font-mono text-ng-on">{avgAccuracy.toFixed(1)}<span className="text-sm text-ng-muted ml-0.5">%</span></div>
 </PanelCard>
 <PanelCard className="flex-1 p-3 flex flex-col gap-1">
 <span className="text-[10px] text-ng-muted font-mono tracking-widest">MODELS SERVING</span>
 <div className="text-2xl font-mono text-ng-on">{servingCount}<span className="text-sm text-ng-muted ml-0.5">/{models.length}</span></div>
 </PanelCard>
 <PanelCard className="flex-1 p-3 flex flex-col gap-1">
 <span className="text-[10px] text-ng-muted font-mono tracking-widest">EVENTS PROCESSED</span>
 <div className="text-2xl font-mono text-ng-on">
 ...
 <div className="absolute top-3 left-3 z-20 bg-ng-mid border border-ng-outline-dim/40 px-2 py-0.5">
 <span className="text-[10px] text-ng-cyan font-mono tracking-widest flex items-center gap-2">
 <span className="w-1.5 h-1.5 bg-ng-cyan-bright animate-pulse-fast border border-ng-base" /> INFERENCE TOPOLOGY
 </span>
 </div>
 {isDemo && (
 <div className="absolute top-2 right-2 z-20 text-[9px] font-mono bg-ng-magenta/10 border border-ng-magenta/30 text-ng-magenta px-1.5 py-0.5">
 [DEMO DATA]
 </div>
 )}
 
 <div className="flex-1 relative border border-ng-outline-dim/40 bg-ng-base overflow-hidden mt-3 md:mt-0">
 ...
 >
 <div className={`w-8 h-8 rounded-none border-[1.5px] ${isPulsing ? 'border-ng-cyan/50 bg-ng-cyan-bright/10 text-ng-cyan' : 'border-ng-outline-dim/40 bg-ng-mid text-ng-muted'} flex items-center justify-center transition-colors duration-100`}>
 <Icon className="w-4 h-4" />
 </div>
 <span className={`mt-1 text-[8px] font-mono tracking-widest px-1 whitespace-nowrap transition-colors duration-100 ${isPulsing ? 'bg-ng-cyan-bright text-ng-base' : 'bg-ng-mid text-ng-muted'}`}>
 {node.label}
 </span>
 </div>
 );
 ...
 <div className="px-3 py-2 border-b border-ng-outline-dim/40 bg-ng-mid">
 <span className="text-[10px] text-ng-muted font-mono tracking-widest uppercase">Stream Telemetry</span>
 </div>
 <div className="flex-1 grid grid-cols-2 lg:grid-cols-3 gap-6 p-4">
 {Object.keys(streams).length === 0 ? (
 <div className="col-span-full h-full flex flex-col items-center justify-center text-ng-muted opacity-50">
 ...
 const scoreColor = score > 0.8 ? 'var(--ng-lime)' : score > 0.6 ? 'var(--ng-magenta)' : 'var(--ng-error)';
 return (
 <div key={key} className="flex flex-col">
 <div className="flex justify-between items-end mb-1">
 <span className="text-[10px] text-ng-muted font-mono tracking-widest uppercase">{STREAM_LABELS[key] ?? key}</span>
 <span className="text-[10px] font-mono" style={{ color: scoreColor }}>{(score * 100).toFixed(1)}%</span>
 </div>
 <div className="w-full h-1 bg-ng-base border border-ng-outline-dim/40 relative">
 ...
 <PanelCard className="flex-1 flex flex-col overflow-hidden">
 <div className="px-3 py-2 border-b border-ng-outline-dim/40 bg-ng-mid">
 <span className="text-[10px] font-mono tracking-widest text-ng-muted uppercase">Model Registry</span>
 </div>
 ...
 rowKey="name"
 columns={[
 { header: "ST", key: "status", render: (val) => <div className={`w-1.5 h-1.5 ${val === 'serving' ? 'bg-ng-lime' : 'bg-ng-magenta'}`} /> },
 { header: "MODEL", key: "name", render: (val) => <div className="flex items-center gap-2"><span className="text-[11px] text-ng-on">{val}</span>{isDemoModels && <span className="text-[8px] border border-ng-magenta/40 text-ng-magenta px-1.5 font-mono">[DEMO]</span>}</div> },
 { header: "ACCURACY", key: "accuracy", align: "right", render: (val) => <span className="text-[11px] font-mono text-ng-on">{val}%</span> }
 ]}
 />
 </div>
 </PanelCard>
 ...
 {/* Neural Log */}
 <PanelCard className="flex-1 flex flex-col overflow-hidden">
 <div className="px-3 py-2 border-b border-ng-outline-dim/40 bg-ng-mid flex justify-between items-center">
 <span className="text-[10px] font-mono tracking-widest text-ng-muted uppercase">Neural Debug Log</span>
 ...
 <div className="w-1.5 h-1.5 bg-ng-cyan-bright animate-pulse-fast" /> SYNCED
 </div>
 </div>
 <div className="flex-1 p-3 overflow-y-auto custom-scrollbar bg-ng-base font-mono text-[9px] uppercase space-y-1.5">
 {neuralLog.map((entry, i) => (
 <div key={i} className="flex gap-2 leading-tight">
 <span className="text-ng-muted shrink-0 w-16">{entry.timestamp}</span>
 <span className={`${entry.status === 'FLAGGED' ? 'text-ng-error' : entry.type === 'WARN' ? 'text-ng-magenta' : 'text-ng-cyan/60'}`}>
 {entry.message}
 </span>
 </div>
 ))}
