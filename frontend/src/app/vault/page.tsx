  ...
  return (
    <div className="flex-1 p-6 overflow-auto custom-scrollbar">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {isLoading ? (
          <div className="animate-pulse bg-sf-surface/60 h-32 rounded-none border border-sf-border" />
        ) : (
          <div className={`sf-panel border ${configured ? 'border-[var(--sf-safe)]/30' : 'border-[var(--sf-warning)]/30'} p-6 rounded-none flex items-center gap-6 shadow-[0_4px_20px_rgba(0,0,0,0.5)] relative overflow-hidden`}>
            ...
            <div className={`w-16 h-16 rounded-none border bg-sf-bg/80 ${configured ? 'border-[var(--sf-safe)]/50 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'border-[var(--sf-warning)]/50 shadow-[0_0_15px_rgba(245,158,11,0.3)]'} flex items-center justify-center flex-shrink-0 z-10`}>
              <Lock className={`w-8 h-8 ${configured ? 'text-[var(--sf-safe)] drop-shadow-[0_0_5px_rgba(16,185,129,0.8)]' : 'text-[var(--sf-warning)]'}`} />
            </div>
            <div className="z-10 flex-1">
              <h3 className="text-xl font-bold text-sf-text">
                Vault Status: <span className={configured ? 'text-[var(--sf-safe)]' : 'text-[var(--sf-warning)]'}>
              ...
              {configured && (
                <div className="mt-4 flex gap-4 text-xs font-mono text-sf-muted">
                  <span className="bg-sf-surface/50 px-2 py-1 rounded-none border border-sf-border">Mount: {mountPoint}</span>
                  <span className="bg-sf-surface/50 px-2 py-1 rounded-none border border-sf-border">Auto-Rotation: Enabled</span>
                </div>
              )}
              {!configured && (
                <div className="mt-5">
