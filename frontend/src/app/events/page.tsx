...
const EventRow = ({ index, style, data }: any) => {
                                ...
                                const colorHex = isCritical ? '#f43f5e' : isWarn ? '#fbbf24' : '#00f2ff';

                                return (
                                    <div style={{ ...style }}>
                                        <div
                                            onClick={() => setSelectedLog(log)}
                                            className={`h-[114px] bg-brand-surface/80 backdrop-blur-md p-4 rounded-xl border-l-[4px] flex flex-col gap-2 cursor-pointer transition-all hover:-translate-y-1 hover:shadow-lg ${isCritical ? 'border-l-[#f43f5e] shadow-[0_0_20px_rgba(244,63,94,0.15)] border border-brand-critical/20' : isWarn ? 'border-l-[#fbbf24] border border-brand-warning/20' : 'border-l-[#00f2ff] border border-brand-accent/10 hover:border-brand-accent/40'}`}
                                        >
                                            ...
                                            {log.meta.length > 0 && (
                                                <div className="flex gap-2 mt-1 flex-wrap">
                                                    {log.meta.map((m: any, i: number) => (
                                                        <span key={i} className={`text-[10px] uppercase font-mono ${isCritical ? 'bg-brand-critical/10 text-brand-critical border-brand-critical/30' : 'bg-brand-dark text-slate-400 border-slate-700'} px-1.5 py-0.5 rounded border`}>
                                                            {m.key}: {m.value}
                                                        </span>
                                                    ))}
                                                </div>
    ...
    return (
        <div className="flex-1 overflow-hidden flex flex-col relative bg-[linear-gradient(rgba(0,242,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,242,255,0.03)_1px,transparent_1px)] bg-[size:30px_30px]">
            ...
            <div className="flex flex-col md:flex-row gap-3 p-6 pb-2 z-10">
                <div className="flex-1 bg-brand-dark/70 backdrop-blur-md border border-brand-accent/50 rounded-xl p-4 shadow-[0_0_15px_rgba(0,242,255,0.15)] relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-brand-accent/5 to-transparent"></div>
                    <p className="text-slate-400 text-xs font-medium uppercase tracking-wider relative">Events/Sec</p>
                    <div className="flex items-end gap-2 mt-1 relative">
                        ...
                        {eps > 0 && <span className="text-brand-accent text-xs font-bold mb-1">LIVE</span>}
                    </div>
                </div>
                <div className="flex-1 bg-brand-dark/70 backdrop-blur-md border border-brand-accent/50 rounded-xl p-4 shadow-[0_0_15px_rgba(0,242,255,0.15)] relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-brand-accent/5 to-transparent"></div>
                    <p className="text-slate-400 text-xs font-medium uppercase tracking-wider relative">Total Events</p>
                    <div className="flex items-end gap-2 mt-1 relative">
                        <span className="text-white text-3xl font-bold leading-none tracking-tight">{liveEvents.length || '0'}</span>
                        {liveEvents.length > 0 && <span className="text-brand-accent text-xs font-bold mb-1">streamed</span>}
                    </div>
                </div>
            </div>

            {/* Stream Label */}
            <div className="px-6 py-4 flex items-center justify-between z-10 border-b border-brand-accent/10 mb-2 bg-brand-dark/50 backdrop-blur-sm">
                <div className="flex items-center gap-2">
                    <span className={`flex size-2.5 rounded-full ${liveEvents.length > 0 ? 'bg-brand-accent animate-pulse shadow-[0_0_8px_#00f2ff]' : 'bg-slate-600'}`}></span>
                ...
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${liveEvents.length > 0 ? 'text-brand-accent bg-brand-accent/10 border-brand-accent/30 animate-pulse' : 'text-slate-500 bg-slate-800 border-slate-700'}`}>
            ...
            {/* Modal */}
            {selectedLog && (
                <div className="absolute inset-x-4 md:inset-x-20 lg:inset-x-32 top-20 z-50 bg-[#0a111a]/95 backdrop-blur-xl rounded-xl p-6 shadow-[0_0_30px_rgba(0,242,255,0.2)] border border-brand-accent/50 animate-[slideDown_0.2s_ease-out]">
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-mono text-slate-400 ml-2">{selectedLog.id}</span>
                        </div>
                    ...
                    <div className="bg-black/80 rounded-lg p-5 font-mono text-[12px] leading-loose border border-brand-accent/20 overflow-x-auto max-h-[400px] overflow-y-auto">
                    ...
                    <div className="mt-5 flex gap-3">
                        <button className="flex-1 bg-brand-accent text-brand-dark font-bold text-xs py-3 rounded-lg flex items-center justify-center gap-2 hover:bg-[#00d0db] hover:shadow-[0_0_15px_#00f2ff] transition-all">
                            <Share2 className="w-4 h-4" /> EXPORT JSON
                        </button>
                        <button className="flex-1 bg-brand-accent/10 border border-brand-accent/40 text-brand-accent hover:bg-brand-accent/20 font-bold text-xs py-3 rounded-lg flex items-center justify-center gap-2 transition-all">
            ...
            {selectedLog && (
                <div
                    className="absolute inset-0 bg-brand-dark/60 backdrop-blur-sm z-40"
