...
const umbrixTheme = EditorView.theme({
 "&": {
 backgroundColor: "var(--ng-base)",
 color: "var(--ng-on)",
 fontSize: "12px",
 },
 ".cm-gutters": {
 backgroundColor: "var(--ng-mid)",
 color: "var(--ng-muted)",
 border: "none",
 borderRight: "1px solid var(--ng-outline-dim/30)",
 },
 ".cm-activeLineGutter": { backgroundColor: "var(--ng-high)" },
 ".cm-activeLine": { backgroundColor: "color-mix(in srgb, var(--ng-mid) 40%, transparent)" },
 ".cm-cursor": { borderLeftColor: "var(--ng-cyan-bright)" },
 ".cm-selectionBackground, ::selection": { backgroundColor: "color-mix(in srgb, var(--ng-cyan-bright) 20%, transparent)" },
}, { dark: true });
...
export function HunterMode({ liveFeed, eventsRate, setMaximizedWidget }: DashboardModeProps) {
 ...
 <PanelCard className="flex-1 p-0 flex items-stretch border-ng-cyan/50/30 overflow-hidden">
 <div className="flex items-center gap-2 px-3 py-2 border-r border-ng-outline-dim/40 shrink-0">
 <Terminal className="w-4 h-4 text-ng-cyan" />
 <span className="text-[9px] font-mono tracking-widest text-ng-muted uppercase">UQL</span>
 </div>
 ...
 className="px-4 flex items-center gap-2 bg-ng-cyan-bright/10 hover:bg-ng-cyan-bright/20 border-l border-ng-outline-dim/40 text-ng-cyan text-[10px] font-mono tracking-widest uppercase transition-colors disabled:opacity-40 shrink-0"
 >
 {isRunning ? (
 ...
 <Activity className="w-5 h-5 text-ng-cyan" />
 <div className="flex flex-col items-end gap-1">
 <span className="text-2xl font-mono text-ng-on leading-none mt-2">
 {eventsRate} <span className="text-xs text-ng-muted">EPS</span>
 </span>
 ...
 <PanelCard className="shrink-0 p-2 flex items-center gap-3 border-ng-cyan/50/10">
 <Search className="w-3.5 h-3.5 text-ng-muted shrink-0 ml-1" />
 <input
 type="text"
 className="flex-1 bg-transparent border-none text-ng-on font-mono text-xs focus:outline-none placeholder:text-ng-muted/40"
 ...
 />
 <span className="text-[9px] font-mono text-ng-muted/50 shrink-0 pr-1">↵ to search</span>
 </PanelCard>

 {queryError && (
 <div className="shrink-0 bg-ng-error/10 border border-ng-error/30 px-3 py-2 text-ng-error text-[10px] font-mono">
 ERROR: {queryError}
 </div>
 )}
 ...
 {/* MITRE heatmap */}
 <PanelCard className="flex-1 relative overflow-hidden flex flex-col p-4 bg-ng-base">
 <div className="mb-4 shrink-0 flex justify-between items-center">
 <h2 className="font-headline tracking-widest uppercase text-[10px] text-ng-muted font-mono tracking-widest uppercase">
 MITRE ATT&CK Enterprise Coverage
 </h2>
 <button 
 onClick={() => setMaximizedWidget('map')} 
 className="text-ng-muted hover:text-ng-on transition-colors bg-ng-base/80 backdrop-blur border border-ng-outline-dim/40 p-1.5 rounded hover:bg-ng-mid"
 >
 ...
 <Cpu className="w-3.5 h-3.5 text-ng-muted mr-2" />
 <span className="text-[10px] font-mono tracking-widest text-ng-muted uppercase">
 ...
 <button
 onClick={() => setResults([])}
 className="text-[9px] font-mono text-ng-muted/60 hover:text-ng-muted transition-colors"
 >
 CLEAR
 </button>
 )}
 ...
 key: 'timestamp',
 render: (val) => (
 <span className="font-mono text-[9px] text-ng-muted">
 ...
 key: 'message',
 render: (val, item: any) => (
 <span className="font-mono text-[9px] text-ng-on truncate block max-w-[200px]">
 {val || item.action || '—'}
 </span>
 ),
 },
